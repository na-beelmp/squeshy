...
bool is_ipv4(const char_t* str, const size_t n)
{
	if (n > 15) {
		return false;
	}
...
int furl_decode(furl_handler_t *fh, const char_t *url, const size_t url_len)
{
	...
	if (!furl_features_errors_lookup(url_features)) {
		...
		if (furl_features_exist(url_features->credential)) { 
			if (furl_features_exist(url_features->scheme)) {
				url_features->credential.pos = url_features->scheme.size + 3;
			} else {
				url_features->credential.pos = 0;
			}
		...
		if (furl_features_exist(url_features->host)) { 
			...
			if (next_valid_token_pos > url_features->host.pos) {
				...
				const char_t* host = url + url_features->host.pos;
				if (!is_ipv4(host, total_size)) {
					/* Extract the TLD now */
					const char_t *tld = (const char_t*) memrchr(host, '.', url_features->host.size*sizeof(char_t));
					if (tld) {
						tld++;
						const uint32_t tld_pos = (uint32_t) (((uintptr_t)tld)-((uintptr_t)host))/sizeof(char_t);
						...
						if (tld_len>1) {
							...
							const char* domain = (const char*) memrchr(host, '.', (url_features->host.size - tld_len - 1)*sizeof(char_t));
							if (domain) {
								uint32_t domain_pos = (uint32_t) (((uintptr_t)domain)-((uintptr_t)host))/sizeof(char_t);
								...
								// If no DOMAIN, the domain is same as the host
...
#ifdef FURL_UTF16_CHAR
	fwprintf(stderr, L"Cannot parse the url: '%s'\n", url);
#else
